...
void addHeap(DynArr *heap, TYPE node)
{
  	/* FIXME */
		...
		while(pos != 0) {
			if(compare(getDynArr(heap, pos), getDynArr(heap, pidx)) == -1) {
				swapDynArr(heap, pidx, pos);
				pos = pidx;
			}
			else {
				return;
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
  	/* FIXME */
		...
		if(rightIdx <= max) {
			...
			if(compare(getDynArr(heap, pos), getDynArr(heap, smallIdx)) == 1) {
...
void removeMinHeap(DynArr *heap)
{
  	/* FIXME */
		...
		_adjustHeap(heap, lastIdx - 1, 0);		/* Run recursion */
}
